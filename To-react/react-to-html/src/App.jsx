...
import { CharacterCards, TopCharacterTableRow } from './character-roster';
...
function App() {
  const highestVoted = data.filter((character) => character.votes >= 90);
  const highestToLowestVoted = highestVoted.sort((a,b) => b.votes - a.votes);
  return (
    <>
      <header>
      ...
      <h4>Top Characters</h4>
        <table>
          ...
          <tbody>
            {highestToLowestVoted.map((character, index) => <TopCharacterTableRow key={character.name} name={character.name} skillset={character.skillset} votes={character.votes} index={index + 1}/>)}
          </tbody>
        </table>
      </section>
      <section id='character-cards'>
        {data.map(character => <CharacterCards key={character.name} charCard={{name: character.name, nickName: character.nickName, imgUrl: character.imageUrl, background: character.background}}/>)}
      </section>
    </>
  )
}
